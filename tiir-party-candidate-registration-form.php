...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate each field individually
        ...
        if (empty($form_data['phone'])) {
            ...
            $errors[] = 'Invalid phone number format';
                ...
                if (!empty($form_data['govt'])) {
                    $gov_check = $conn->prepare("SELECT id FROM candidates WHERE government_id = ?");
                    if ($gov_check) {
        ...
        // Photo is required for candidate registration
    ...
    if (empty($errors)) {
        ...
        $photo_path = null;
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            // store candidate photos under uploads/candidates/
            $photo_path = upload_file($_FILES['photo'], 'uploads/candidates/');
            if (!$photo_path) {
                $errors[] = 'Photo upload failed';
            }
        } else {
            // If it wasn't uploaded correctly, ensure we don't proceed
            if (empty($photo_path)) {
                $errors[] = 'Photo is required';
            }
        }
        ...
        if (empty($errors)) {
            $stmt = $conn->prepare("INSERT INTO candidates (first_name, mothers_name, gender, date_of_birth, place_of_birth, government_id, education, occupation, country, state, district, email, phone, photo_path, status, security_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)");
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    $success = 'Candidate Registration Successful! Your information has been submitted.';
                    // For standard (non-AJAX) submissions keep existing session flash behavior
                    if (!$is_ajax) {
                        $_SESSION['flash_success'] = $success;
                    }
                    // Reset form data and rotate security code
                    $form_data = [];
                    $_SESSION['security_code'] = rand(10000, 99999);

                    // For AJAX submissions return JSON immediately
                    if ($is_ajax) {
...
<div id="content">
  <div id="main" class="inner_main">
    ...
    <div class="post">
      <h1><span>TIIR Party Candidate Registration Form</span></h1>
      <div class="body">
      ...
      <div class="form-container">
        ...
        <form class="classic-form" method="POST" enctype="multipart/form-data" id="candidate-form">
...
<script>
// AJAX submit for candidate registration to show admin-style popup without full page reload
(function(){
    ...
    form.addEventListener('submit', function(e){
        ...
        fetch(window.location.href, {
                                                                                        ...
                                                                                        if (data.form_data) {
                                                                                            try {
                                                                                                Object.keys(data.form_data).forEach(function(k){
                                                                                                    ...
                                                                                                    // Try to find a hidden field (e.g., dob hidden ISO input inserted by flatpickr)
                                                                                                    ...
                                                                                                    if (hidden) {
                                                                                                        ...
                                                                                                        // Try to update the visible flatpickr input using flatpickr instance if available
                                                                                                        // Visible input is often previousElementSibling of the hidden input
                                                                                                        var vis = hidden.previousElementSibling;
                                                                                                        if (vis) {
                                                                                                            // If value is a date in Y-m-d, create a dd/mm/YYYY string for visible field
                                                                                                            ...
                                                                                                            // If flatpickr instance is attached to either vis or the original input, call setDate
                                                                                                            var instance = vis._flatpickr || hidden._flatpickr || vis._fp || null;
                                                                                                            if (!instance) {
                                                                                                                // try to find candidate input with data attribute
                                                                                                            ...
                                                                                                            if (instance && k === 'dob' && v) {
                                                                                                                try { instance.setDate(v, true); } catch(e) { /* ignore */ }
                                                                                                            }
                                                                                                        }
                                                                                                    }
                                                                                                });
            ...
            // Not JSON — read text. If server returned HTML (likely non-AJAX full POST), reload to allow flash messages.
            return response.text().then(function(text){
                if (submitBtn) submitBtn.disabled = false;
                if (response.ok) {
                    // Let server-rendered page handle flash messages (non-AJAX fallback).
