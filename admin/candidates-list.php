...
<?php
// Similar to members-list.php but for candidates
...
// Get total count
$count_query = "SELECT COUNT(*) as total FROM candidates WHERE $where_clause";
...
// Get candidates
$query = "SELECT * FROM candidates WHERE $where_clause ORDER BY registration_date DESC LIMIT $offset, $per_page";
...
$countries_query = "SELECT DISTINCT country FROM candidates ORDER BY country";
...
?>
                ...
                <form method="GET" class="filters-container">
                    ...
                    <div class="filter-buttons" style="margin-top: 15px;">
                        ...
                        <a href="candidates-list.php" class="reset-filter">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td>
                                                <strong><?php echo htmlspecialchars($candidate['first_name'] . ' ' . $candidate['mothers_name']); ?></strong>
                                            </td>
                        ...
                        <?php if ($total_pages > 1): ?>
                            <div class="pagination-container">
                                ...
                                <div class="pagination-links">
                                    <?php if ($page > 1): ?>
                                        <a href="?page=1<?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?><?php echo !empty($country_filter) ? '&country=' . urlencode($country_filter) : ''; ?>" class="page-link">« First</a>
                                        <a href="?page=<?php echo $page - 1; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?><?php echo !empty($country_filter) ? '&country=' . urlencode($country_filter) : ''; ?>" class="page-link">‹ Prev</a>
                                    <?php endif; ?>
                                    ...
                                    for ($i = $start; $i <= $end; $i++):
                                    ?>
                                        <a href="?page=<?php echo $i; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?><?php echo !empty($country_filter) ? '&country=' . urlencode($country_filter) : ''; ?>" class="page-link <?php echo $i === $page ? 'active' : ''; ?>">
                                            <?php echo $i; ?>
                                        </a>
                                    <?php endfor; ?>

                                    <?php if ($page < $total_pages): ?>
                                        <a href="?page=<?php echo $page + 1; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?><?php echo !empty($country_filter) ? '&country=' . urlencode($country_filter) : ''; ?>" class="page-link">Next ›</a>
                                        <a href="?page=<?php echo $total_pages; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?><?php echo !empty($country_filter) ? '&country=' . urlencode($country_filter) : ''; ?>" class="page-link">Last »</a>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endif; ?>
