...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($form_data['email']) && $form_data['email'] !== $member['email']) {
        $email_check = $conn->prepare("SELECT id FROM members WHERE email = ? AND id != ?");
        if ($email_check) {
            $email_check->bind_param("si", $form_data['email'], $member_id);
            $email_check->execute();
            if ($email_check->get_result()->num_rows > 0) {
                $errors[] = 'Email already registered to another member';
            }
            $email_check->close();
        }
    }
    ...
    if ($form_data['phone'] !== $member['phone']) {
        $phone_check = $conn->prepare("SELECT id FROM members WHERE phone = ? AND id != ?");
        if ($phone_check) {
            $phone_check->bind_param("si", $form_data['phone'], $member_id);
            $phone_check->execute();
            if ($phone_check->get_result()->num_rows > 0) {
                $errors[] = 'Phone number already registered to another member';
            }
            $phone_check->close();
        }
    }
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        $new_photo = upload_file($_FILES['photo'], '../uploads/members/');
        if ($new_photo) {
            // Delete old photo
    ...
    if (empty($errors)) {
        $update_query = $conn->prepare(
            "UPDATE members SET first_name=?, mothers_name=?, gender=?, date_of_birth=?,
...
?>
                ...
                <div class="form-container-admin">
                    <h1 class="page-title"><i class="fas fa-edit" style="margin-right: 10px;"></i>Edit Member</h1>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-buttons">
                            ...
                            <a href="members-list.php" class="btn btn-secondary">
