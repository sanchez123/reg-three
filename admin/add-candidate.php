...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($form_data['gender']) || !in_array($form_data['gender'], ['Male', 'Female'])) $errors[] = 'Valid gender selection is required';
    ...
    if (empty($form_data['education']) || !in_array($form_data['education'], ['Primary School', 'Secondary School', 'Diploma', 'Degree'])) $errors[] = 'Valid education level is required';
    ...
    if (empty($errors)) {
        $status = 'pending';
        $admin_by = intval($_SESSION['admin_id']);
        ...
        if ($insert_query) {
            ...
            if ($insert_query->execute()) {
                log_audit($admin_by, 'CREATE', 'candidates', $insert_query->insert_id, ['name' => $form_data['first_name'] . ' ' . $form_data['mothers_name']]);
...
.form-group input, .form-group select, .form-group textarea { padding: 12px 15px; border: 2px solid #ddd; border-radius: 8px; font-size: 15px; font-family: Arial, sans-serif; transition: 0.3s ease; }
...
<div class="form-container-admin">
    ...
    <form method="POST" enctype="multipart/form-data">
        <div class="form-row">
            ...
            <div class="form-group">
                <label>Mother's Name <span style="color:#dc2626;">*</span></label>
        ...
        <div class="form-row">
            <div class="form-group">
                <label>Education Level <span style="color:#dc2626;">*</span></label>
                <select name="education" required>
                    <option value="">Select Education Level</option>
                    <option value="Primary School" <?php echo (isset($form_data['education']) && $form_data['education'] === 'Primary School') ? 'selected' : ''; ?>>Primary School</option>
                    <option value="Secondary School" <?php echo (isset($form_data['education']) && $form_data['education'] === 'Secondary School') ? 'selected' : ''; ?>>Secondary School</option>
                    <option value="Diploma" <?php echo (isset($form_data['education']) && $form_data['education'] === 'Diploma') ? 'selected' : ''; ?>>Diploma</option>
                    <option value="Degree" <?php echo (isset($form_data['education']) && $form_data['education'] === 'Degree') ? 'selected' : ''; ?>>Degree</option>
                </select>
            </div>
            <div class="form-group">
                <label>Occupation <span style="color:#dc2626;">*</span></label>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="country" required>
                    ...
                    <option value="Somalia" <?php echo (isset($form_data['country']) && $form_data['country'] === 'Somalia') ? 'selected' : ''; ?>>Somalia</option>
                    <option value="Kenya" <?php echo (isset($form_data['country']) && $form_data['country'] === 'Kenya') ? 'selected' : ''; ?>>Kenya</option>
                    <option value="Ethiopia" <?php echo (isset($form_data['country']) && $form_data['country'] === 'Ethiopia') ? 'selected' : ''; ?>>Ethiopia</option>
                </select>
            </div>
            <div class="form-group">
                <label>State/Region <span style="color:#dc2626;">*</span></label>
